                ...
                <div className="med-cards-box">
                        ...
                        <div className="pets-list">
                            <div className="doctor-mode-toggle">
                                <label className="doctor-label">
                                    <input type="checkbox" checked={isDoctorMode} onChange={(e) =>
                                        setIsDoctorMode(e.target.checked)} />
                                    Режим врача
                                </label>
                            </div>
                            ...
                            <div className="list-top-bar">
                                ...
                                <span className="logout-link" onClick={() => {setIsAuthorized(false);
                                    setViewMode('search'); setPhone('');}}>Выйти</span>
                            </div>
                                ...
                                <div key={pet.id} className="pet-card">
                                    <div className="pet-header" onClick={() => togglePet(pet.id)}>
                                        <div>
                                            ...
                                            <div className="pet-info-short">
                                                {editingPetId === pet.id
                                                    ? <span className="editing-text">Редактирование...</span>
                                        ...
                                        <div className="header-actions">
                                            {isDoctorMode && editingPetId !== pet.id && (
                                                <button onClick={(e) => startEditing(e, pet)} className="edit-pencil-btn">✏️</button>
                                            )}
                                    ...
                                    <div className={`pet-details ${expandedPetId === pet.id ? 'open' : ''}`}>
                                        <div className="details-content">
                                            {editingPetId === pet.id ? (
                                                <div className="edit-container">
                                                    <div className="stats-grid">
                                                        <div className="stat-item"><span>Вид</span><input name="type" className="edit-input"
                                                                                                          value={editFormData.type} onChange={handleEditChange} /></div>
                                                        <div className="stat-item"><span>Порода</span><input name="breed" className="edit-input"
                                                                                                             value={editFormData.breed} onChange={handleEditChange} /></div>
                                                        <div className="stat-item"><span>Возраст</span><input name="age" className="edit-input"
                                                                                                              value={editFormData.age} onChange={handleEditChange} /></div>
                                                        <div className="stat-item"><span>Вес</span><input name="weight" className="edit-input"
                                                                                                          value={editFormData.weight} onChange={handleEditChange} /></div>
                                                        <div className="stat-item full-width"><span>Особенности</span>
                                                            <textarea name="past_injuries" className="edit-textarea" rows="3"
                                                                      value={editFormData.past_injuries} onChange={handleEditChange} /></div>
                                                    </div>
                                                    <div className="edit-buttons-row">
                                                        <button className="save-pet-btn cancel" onClick={cancelEditing}>Отмена</button>
                                            ...
                                            {loadingHistory ? (
                                                ...
                                                activeHistory.map(app => (
                                                    <div key={app.id} className="history-item">
                                                        ...
                                                        <div className="history-complaint">Обращение: {app.comment || "Нет"}</div>

                                                        <div className="diagnosis-box">
                                                            <span className="diagnosis-label">Врачебное заключение:</span>
                                                            {app.diagnosis ?
                                                                <div className="diagnosis-content">{app.diagnosis}</div> :
                                                                <div className="diagnosis-empty">Ожидает заполнения...</div>
                                                            }

                                                            {isDoctorMode && (
                                                                <div className="doctor-input-area">
                                                                    <textarea className="edit-textarea" placeholder="Введите диагноз..."
                                                                              value={diagnosisInputs[app.id] !== undefined ? diagnosisInputs[app.id] : (app.diagnosis || '')}
                                                                              onChange={(e) =>
                                                                                  handleDiagnosisChange(app.id, e.target.value)} />
                                            ...
                                            <button className="delete-card-btn" onClick={(e) =>
                                            { e.stopPropagation(); handleDeleteCard(pet.id); }}>Удалить медицинскую карту</button>
                                        </div>
                                    </div>
                                </div>
                            ))}
                    ...
                    {viewMode === 'create' && (
                        <form className="appointment-form" onSubmit={handleCreateSubmit}>
                            ...
                            <div className="form-group"><label>Телефон владельца</label><input type="tel" value={phone}
                                                                                               onChange={handlePhoneChange} required /></div>
                            <div className="form-group"><label>Кличка</label><input name="name" value={newPetData.name}
                                                                                    onChange={handleCreateInput} required /></div>
                            <div className="form-group"><label>Кто это?</label>
                                <select name="type" value={newPetData.type}
                            ...
                            {newPetData.type === 'other' &&
                                (<div className="form-group"><input name="customType" placeholder="Вид животного" value={newPetData.customType}
                                                                                                onChange={handleCreateInput} required /></div>)}
                            <div className="form-group"><label>Порода</label><input name="breed" value={newPetData.breed}
                                                                                    onChange={handleCreateInput} /></div>
                            <div className="form-row">
                                <div className="form-group form-col"><label>Возраст</label><input name="age" value={newPetData.age}
                                                                                                  onChange={handleCreateInput} /></div>
                                <div className="form-group form-col"><label>Вес</label><input name="weight" value={newPetData.weight}
                                                                                              onChange={handleCreateInput} /></div>
                            </div>

                            <div className="form-group"><label>Хронические болезни</label>
                                <textarea name="past_injuries" value={newPetData.past_injuries} onChange={handleCreateInput} rows="2" /></div>
                            <div className="form-actions"><button type="button" onClick={() =>
                                setViewMode('search')} className="submit-btn cancel-btn">
                                Отмена</button>
                                <button type="submit" className="submit-btn">Создать карту</button></div>
                        </form>
                    )}
                </div>
            </main>
